...
async function main() {
  ...
  // Same tiny amounts as airdrop-eth.ts
  const amounts = [
  ...
  const drop = await MultiSendETH.deploy();
  await drop.waitForDeployment();
  const dropAddress = await drop.getAddress();
  console.log("MultiSendETH deployed at:", dropAddress);

  const airdropTx = await drop.multisendETH(recipients, amounts, {
    value: total,
  });
  const receipt = await airdropTx.wait();
  console.log("Native CELO airdrop (regular) complete. Tx:", airdropTx.hash);
